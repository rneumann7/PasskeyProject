...
func newRSASigningKey() (*rsaSigningKey, []byte) {
	...
	return newRSASigningKeyWithPrivateKey(privateKey, false), keyData
}

func importRSASigningKey(keyData []byte, wrongAlg bool) *rsaSigningKey {
	...
	return newRSASigningKeyWithPrivateKey(privateKey, wrongAlg)
}

func newRSASigningKeyWithPrivateKey(privateKey *rsa.PrivateKey, wrongAlg bool) *rsaSigningKey {
	var usedAlg int64 = rsaSHA256Algo
	if wrongAlg {
		usedAlg = 0
	}
	info := rasKeyInfo{
